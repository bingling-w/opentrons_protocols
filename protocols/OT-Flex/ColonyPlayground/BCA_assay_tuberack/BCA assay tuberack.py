...
metadata = {
    'protocolName': 'BCA Protein Assay Kit Tube Version',
    'author': 'OpentronsAI',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    standard_tube_rack = protocol.load_labware('opentrons_24_tuberack_eppendorf_1.5ml_safelock_snapcap', 'D1')
    unknown_tube_rack = protocol.load_labware('opentrons_24_tuberack_eppendorf_1.5ml_safelock_snapcap', 'D2')
    ...
    standard_tube_rack['D6'].load_liquid(liquid=reagent_b, volume=1500)
    ...
    for tube in standard_sources:
        tube.load_liquid(liquid=bsa_standard, volume=70)
    ...
    # Helper function to calculate destination wells
    import math
    ...
    for idx, source in enumerate(unknown_sources):
        ...
        for dest in destinations:  # List of destinations
    ...
    for replicate in range(replicates_number * columns_per_replicate):
        ...
        p1000_multi.transfer(
            200,
            reservoir['A2'],
            hs_plate.columns()[unknown_dest_column][0],
            blow_out=True,
            blowout_location='destination well',
            new_tip='always'
        )
